"""Cheviplus Photo Studio 5.5: two proven local processing modes."""
...
def _cache_key_from_source(source,mode):
 ...
 try:
  path=Path(source).resolve(); stat=path.stat()
...
class AIQualityApp(bq.BackdropQualityApp):
 def _build(self):
  ...
  self._ai_busy=False; self._ai_busy_phase=0; self.ai_quality_var=tk.StringVar(value=MODE_FAST)
