...
def _text(widget):
    try:return str(widget.cget("text") or "").strip()
...
def _find_main_settings(root):
    ...
    for node in _walk(root):
        if isinstance(node, ttk.LabelFrame):
            ...
            if "стабильная ручная обработка" in title:continue
            if "основн" in title and "настрой" in title:return node
            if "настрой" in title:candidates.append(node)
...
def _add_main_ai_selector(self):
    ...
    combo.pack(side="left",padx=(8,0)); self.ai_quality_main_combo=combo


def build(self):
    _ORIGINAL_BUILD(self)
    if not hasattr(self,"ai_quality_var"):self.ai_quality_var=tk.StringVar(master=self,value=aiq.MODE_FAST)
    _hide_old_ai_controls(self); _add_main_ai_selector(self)


def base_without_generic_category(title,page,cat):
    normalized=str(cat or "").strip().lower()
    if normalized in _GENERIC_CATEGORIES:
        im=visual.Image.new('RGB',visual.SIZE,visual.BG); d=visual.ImageDraw.Draw(im)
        d.rectangle((0,0,1200,122),fill=visual.DARK); d.text((64,36),title,font=visual.font(42,1),fill='white')
        d.rounded_rectangle((1080,28,1144,92),12,fill=visual.NAVY); d.text((1100,42),str(page),font=visual.font(30,1),fill='white')
        return im,d
...
app.APP_VERSION=APP_VERSION; app.APP_BUILD=APP_BUILD
visual.APP_VERSION=APP_VERSION; visual.APP_BUILD=APP_BUILD
workspaces.APP_VERSION=APP_VERSION; workspaces.APP_BUILD=APP_BUILD
